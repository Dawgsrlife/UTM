...
(define check1 (list (make-player 'a (list 5 5))   ; total 10
                     (make-player 'b (list 3 2))    ; total 5
...
(define all-tied
  (list (make-player 'x (list 5 5))
        (make-player 'y (list 10))
        (make-player 'z (list 4 4 2))))

(check-expect (classify-standings all-tied 1)
              (list (list 'y 'tied)))

;; Test: Two players, one winner, one needs improvement
(define two-players
  (list (make-player 'm (list 8 7))
        (make-player 'n (list 3 4))))

(check-expect (classify-standings two-players 2)
              (list (list 'm 'winner)
                    (list 'n 'needs-improvement)))

;; Test: Multiple tied winners
(define tied-winners
  (list (make-player 'p (list 10 10))
        (make-player 'q (list 10 10))
        (make-player 'r (list 5 5))))

(check-expect (classify-standings tied-winners 2)
              (list (list 'p 'winner)
                    (list 'q 'winner)
                    (list 'r 'needs-improvement)))
